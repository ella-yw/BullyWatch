...
<form method = "post" action="" >
  ...
  <div style = 'background-color: lightblue; width:100%; hieght:30px'><h1 style='text-align:center'>SCHOOL-WISE FAKE COMPLAINTS</h1></div>
...
$query = "SELECT COUNT( * ) FROM incident_reporting JOIN `action_taking` USING(report_id) JOIN `un_incident_reporting_bully` USING(report_id) WHERE bully_school_id = '".$location."' AND fake_complaint = 'YES'";
$result = mysqli_query($db, $query); 
...
var colorCharacters = "ACDEF0123456789";
    ...
    var myConfig = {
     	type: "bar", 
     	"scale-x":{
			"values":['<?php echo $school_name . ' (Your School Disp. Only)'; ?>'],
		},
		series : [
    		{
...
<?php
$query = "SELECT COUNT( * ) FROM incident_reporting JOIN `un_incident_reporting_bully` USING(report_id) WHERE bully_school_id = '".$location."'"; 
...
$query = "SELECT COUNT( * ) FROM incident_reporting JOIN `un_incident_reporting_bully` USING(report_id) JOIN `action_taking` USING(report_id) WHERE bully_school_id = '".$location."' AND fake_complaint = 'YES'";
...
$numresults = mysqli_num_rows($result);			
			for ($i = 1; $i <= $numresults; $i++)
			{
				...
				$query = "SELECT COUNT( * )
FROM incident_reporting JOIN `un_incident_reporting_bully` USING(report_id) JOIN `action_taking` USING(report_id) WHERE bully_school_id = '".$location."' AND incident_reporting.bullying_type = '".$row['sub_bullying_type']."' AND fake_complaint = 'YES'";
				$result_a = mysqli_query($db, $query); 
				$row_a = mysqli_fetch_row($result_a);
