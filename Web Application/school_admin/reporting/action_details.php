...
<br/>
  ...
  <promo>
    <h4 style = "color:black;margin-left:20px"> Choose Action ID:
      <select id = 'report-list' name = 'report_id' onChange="showInfo(this.value)" required>
        <?php
require_once("dbcontroller.php");
$db_handle = new DBController();
	$query ="SELECT * FROM `action_taking` JOIN un_incident_reporting_bully USING (`report_id`) WHERE bully_school_id = " . $location. "";
	$results = $db_handle->runQuery($query);
?>
