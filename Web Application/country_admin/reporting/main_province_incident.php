...
<head>
<meta charset="utf-8">
<title>Province-Wise Incident Ratio & Comparison</title>
...
<form method = "post" action="" >
  ...
  <div style = 'background-color: lightblue; width:100%; hieght:30px'><h1 style='text-align:center'>PROVINCE-WISE INCIDENT RATIO & COMPARISON</h1></div>
...
$numresults = mysqli_num_rows($result);			
			for ($i = 1; $i <= $numresults; $i++)
			{
				...
				$query = "SELECT COUNT( * ) FROM incident_reporting JOIN `un_incident_reporting_bully` USING(report_id) JOIN `school` ON (bully_school_id=school_id) JOIN `sd` USING (sd) JOIN `city` ON (city_id=city_name) JOIN `province` USING (province_id) WHERE province_id = '".$row['province_id']."'";
