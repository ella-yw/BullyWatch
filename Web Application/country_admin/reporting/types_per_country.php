...
<form method = "post" action="" >
  ...
  <div style = 'background-color: lightblue; width:100%; hieght:30px'><h1 style='text-align:center'>COUNTRY-WISE BULLYING TYPE RATIO</h1></div>
 </form>
<div id = 'maindiv'>
	 <?php
...
$numresults = mysqli_num_rows($result);			
			for ($i = 1; $i <= $numresults; $i++)
			{
				...
				$query = "SELECT COUNT( * )
FROM incident_reporting JOIN `un_incident_reporting_bully` USING(report_id) JOIN school ON (bully_school_id=school_id) JOIN sd ON (school.sd=sd.sd) JOIN city ON (city_id=city_name) JOIN `province` USING(province_id)
WHERE country_id = '".$location."' AND incident_reporting.bullying_type = '".$row['sub_bullying_type']."'";
...
var myConfig = {
 	type: "pie3d", 
		...
		subtitle: {
		"text":"Total Bullying Incidents In Country: <?php echo $row_cnt; ?>",
		"color":"blue"
		}, 
     	series : [<?php
				$print = '';
