...
WHERE country_id = '".$location."' AND bully_gender = '$gender'"; 
...
$query = "SELECT COUNT( * )
FROM incident_reporting JOIN `un_incident_reporting_bully` USING(report_id) JOIN school ON (bully_school_id=school_id) JOIN sd ON (school.sd=sd.sd) JOIN city ON (city_id=city_name) JOIN `province` USING(province_id)
WHERE country_id = '".$location."' AND incident_reporting.bullying_type = 'General Verbal Insults' AND bully_gender = '$gender'";
...
$query = "SELECT COUNT( * )
FROM incident_reporting JOIN `un_incident_reporting_bully` USING(report_id) JOIN school ON (bully_school_id=school_id) JOIN sd ON (school.sd=sd.sd) JOIN city ON (city_id=city_name) JOIN `province` USING(province_id)
WHERE country_id = '".$location."' AND incident_reporting.bullying_type = 'General Beatings/Pushing' AND bully_gender = '$gender'";
	$result = mysqli_query($db, $query); 
    $row = mysqli_fetch_row($result);
	array_push($pieChart, $row[0]);
...
$query = "SELECT COUNT( * )
FROM incident_reporting JOIN `un_incident_reporting_bully` USING(report_id) JOIN school ON (bully_school_id=school_id) JOIN sd ON (school.sd=sd.sd) JOIN city ON (city_id=city_name) JOIN `province` USING(province_id)
WHERE country_id = '".$location."' AND incident_reporting.bullying_type = 'Terrorizing/Threatening Remarks' AND bully_gender = '$gender'";
	$result = mysqli_query($db, $query); 
    $row = mysqli_fetch_row($result);
	array_push($pieChart, $row[0]);
...
window.onload=function(){
	...
	"type":"ring3d", 
	"title": {
  "text":"Showing Bully Results For: <?php echo $country; ?> (<?php echo $gender; ?>)",
  "color":"green"
