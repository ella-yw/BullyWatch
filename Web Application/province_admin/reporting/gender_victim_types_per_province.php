...
WHERE province_id = '".$location."' AND victim_gender = '$gender'"; 
...
$query = "SELECT COUNT( * )
FROM incident_reporting JOIN `un_incident_reporting_victim` USING(report_id) JOIN school ON (victim_school_id=school_id) JOIN sd ON (school.sd=sd.sd) JOIN city ON (city_id=city_name) JOIN `province` USING(province_id)
WHERE province_id = '".$location."' AND incident_reporting.bullying_type = 'General Verbal Insults' AND victim_gender = '$gender'";
...
$query = "SELECT COUNT( * )
FROM incident_reporting JOIN `un_incident_reporting_victim` USING(report_id) JOIN school ON (victim_school_id=school_id) JOIN sd ON (school.sd=sd.sd) JOIN city ON (city_id=city_name) JOIN `province` USING(province_id)
WHERE province_id = '".$location."' AND incident_reporting.bullying_type = 'General Beatings/Pushing' AND victim_gender = '$gender'";
	$result = mysqli_query($db, $query); 
    $row = mysqli_fetch_row($result);
	array_push($pieChart, $row[0]);
...
$query = "SELECT COUNT( * )
FROM incident_reporting JOIN `un_incident_reporting_victim` USING(report_id) JOIN school ON (victim_school_id=school_id) JOIN sd ON (school.sd=sd.sd) JOIN city ON (city_id=city_name) JOIN `province` USING(province_id)
WHERE province_id = '".$location."' AND incident_reporting.bullying_type = 'Terrorizing/Threatening Remarks' AND victim_gender = '$gender'";
	$result = mysqli_query($db, $query); 
    $row = mysqli_fetch_row($result);
	array_push($pieChart, $row[0]);
...
window.onload=function(){
	...
	"title": {
  "text":"Showing Victim Results For: <?php echo $province; ?> (<?php echo $gender; ?>)",
  "color":"green"
