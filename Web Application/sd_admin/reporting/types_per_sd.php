...
<form method = "post" action="" >
  ...
  <div style = 'background-color: lightblue; width:100%; hieght:30px'><h1 style='text-align:center'>SD-WISE BULLYING TYPE RATIO</h1></div>
...
JOIN school ON (bully_school_id=school_id) WHERE sd = ".$sd."";
...
$numresults = mysqli_num_rows($result);			
			for ($i = 1; $i <= $numresults; $i++)
			{
				...
				$query = "SELECT COUNT( * )
FROM incident_reporting JOIN `un_incident_reporting_bully` USING(report_id) JOIN school ON (bully_school_id=school_id) JOIN sd USING (sd) WHERE sd = '".$location."' AND incident_reporting.bullying_type = '".$row['sub_bullying_type']."'";
...
var myConfig = {
 	type: "pie3d", 
     	title: {
		"text":"Showing Results For: SD <?php echo $sd; ?>",
		"color":"green"

		},
		subtitle: {
		"text":"Total Bullying Incidents In SD: <?php echo $row_cnt; ?>",
		"color":"blue"
		}, 
     	series : [<?php
				$print = '';
